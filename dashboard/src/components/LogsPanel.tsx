      ...
      <div className="h-48 sm:h-64 overflow-y-auto">
        {logs.length === 0 ? (
          <div className="flex items-center justify-center h-full text-gray-400 px-4">
            <p className="text-xs sm:text-sm text-center">{t('logsWillAppear')}</p>
          </div>
        ) : (
          <div className="divide-y divide-gray-100 dark:divide-gray-700">
            {logs.map((log) => {
              ...
              return (
                <div
                  key={log.id}
                  className={`px-3 sm:px-4 py-2 ${config.bg} transition-all animate-in fade-in slide-in-from-top-2 duration-300`}
                >
                  <div className="flex items-start gap-2 sm:gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                        ...
                        <span className="text-[10px] sm:text-xs px-1 sm:px-1.5 py-0.5 rounded bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-400 truncate max-w-[100px] sm:max-w-none">
                          {log.agent}
                        </span>
                      </div>
                      <p className="text-xs sm:text-sm text-gray-700 dark:text-gray-300 mt-0.5 break-words">
                        {log.message}
                      </p>
                    </div>
                  </div>
